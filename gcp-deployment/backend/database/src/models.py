...
class Database:
    ...
    def __init__(self, instance_connection_name: str = None,
                 database: str = None):
        """Initialize database with all model classes (GCP Cloud SQL only)"""
        import os
        
        # GCP Cloud SQL configuration
        instance_connection_name = instance_connection_name or os.environ.get("INSTANCE_CONNECTION_NAME")
        db_password_secret_id = os.environ.get("DB_PASSWORD_SECRET_ID")
        
        if not instance_connection_name and not db_password_secret_id:
            raise ValueError(
                "INSTANCE_CONNECTION_NAME or DB_PASSWORD_SECRET_ID must be set for Cloud SQL connection. "
                "This is a GCP-only deployment."
            )
        
        self.client = CloudSQLClient(
            instance_connection_name=instance_connection_name,
            database=database or os.environ.get("DATABASE_NAME", "alex"),
